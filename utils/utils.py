...
@nb.njit(cache=True)
def get_barycentric_weights_and_indices(points: np.ndarray, bounds_low: np.ndarray, 
                                        bounds_high: np.ndarray, grid_shape: np.ndarray, 
                                        strides: np.ndarray, corner_bits: np.ndarray) -> tuple:
    """
    O(1) Barycentric interpolation weights and flat indices for N-dimensional regular grids.
    Bypasses expensive algorithmic searches in favor of direct mathematical mapping.
    """
...
def evaluate_policy_step(lambdas: np.ndarray, point_indexes: np.ndarray, 
                         value_function: np.ndarray, reward: np.ndarray, 
                         policy: np.ndarray, gamma: float, 
                         new_val: np.ndarray, terminal_mask: np.ndarray) -> None:
    """
    Zero-allocation in-place Bellman expectation update.
...
def get_optimal_action(state: np.ndarray, optimal_policy: any) -> tuple:
    """
    Approximate the optimal action for a given state using multilinear interpolation.
    """
    ...
    if not np.isclose(np.sum(probabilities), 1.0, atol=1e-2):
        raise ValueError(f"Interpolated probabilities do not sum to 1. Sum: {np.sum(probabilities)}")
...
def plot_gradient_regions(bins_space: dict, states_space: np.ndarray, value_function: np.ndarray,
                          quantile: float = 0.90, env=None, normalize: bool = False,
                          show: bool = False, figsize: tuple = (12, 8)) -> tuple:
    """Visualizes regions of high and low gradients in a value function."""
    ...
    if normalize:
        value_nd = (value_nd - value_nd.min()) / (value_nd.max() - value_nd.min() + 1e-8)
    ...
    if show:
        ...
        plt.scatter(low_pts[:, 0], low_pts[:, 1], c='green', s=10, alpha=0.5,
                    label=f'Low-Grad (bottom {100*(1-quantile):.0f}%)')
        plt.scatter(high_pts[:, 0], high_pts[:, 1], c='red', s=30, marker='x', 
                    linewidths=1.5, label=f'High-Grad (top {100*quantile:.0f}%)')
        ...
        if env is not None:
            plt.xlim(getattr(env, f"min_{dim_names[0]}", states_space[:, 0].min()),
                     getattr(env, f"max_{dim_names[0]}", states_space[:, 0].max()))
            plt.ylim(getattr(env, f"min_{dim_names[1]}", states_space[:, 1].min()),
...
def plot_3D_value_function(vf: np.ndarray, points: np.ndarray, normalize: bool = True, 
                           cmap: str = 'turbo_r', show: bool = False, path: str = '') -> None:
    """Plots a 3D value function in a color scale."""
    X = points[:, 0]
    Y = points[:, 1]
...
def plot_2D_value_function_with_cuts(pi: any, bins_space: dict, x_bin_name: str, y_bin_name: str,
                                     fixed_values: dict = None, cmap: str = "hot_r", 
                                     levels: int = 50, method: str = "nearest") -> None:
    """Plots a 2D cross-section of an N-dimensional value function."""
...
def test_environment(task: gym.Env, pi: any, num_episodes: int = 10000, 
                     episode_length: int = 1000, option_reset: dict = None) -> None:
    """Tests the environment using the provided policy."""
    for episode in range(num_episodes):
        ...
        for timestep in range(1, episode_length):
            ...
            if terminated or timestep == episode_length - 1:
                print(f"Episode {episode} finished after {timestep} timesteps | Total reward: {total_reward}")
                break