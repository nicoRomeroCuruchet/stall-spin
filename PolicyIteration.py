...
class PolicyIteration:
    ...
    def _allocate_tensors(self) -> None:
        """Allocate strictly C-contiguous tensors to guarantee cache locality."""
        ...
        self.policy = np.full(
            (self.n_states, self.n_actions), 1.0 / self.n_actions, dtype=np.float32
        )

        self.terminal_mask, terminal_rewards = self.env.terminal(self.states_space)

        # FIX: Handle scalar vs array return for terminal_rewards safely
    ...
    def policy_evaluation(self) -> float:
        ...
        msg = f"Evaluation hit max iterations ({self.config.maximum_iterations})"
        logger.warning(f"{msg} with Δ={delta:.5e}")
        return delta

    def policy_improvement(self) -> bool:
        """Greedily improve the policy with respect to the current value function."""
        ...
        if not policy_stable:
            ...
            logger.info(
                f"Policy updated: {changes} states changed their optimal action."
            )
    ...
    def run(self) -> None:
        """Execute the complete Policy Iteration algorithm."""
